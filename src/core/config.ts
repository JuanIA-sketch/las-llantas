/**
 * Persistencia de `.llantas.json` (§ "Qué guarda .llantas.json") y armado del
 * probe de detección. Archivo local SIN datos sensibles, seguro de commitear:
 * solo el type recordado, la confirmación de identidad npm, y los punteros de
 * "primera corrida / rollback". El I/O (fs) se inyecta para poder testear sin disco.
 *
 * `buildDetectProbe` es quien le pasa `rememberedType` a `detectType`, de modo que
 * detect.ts se mantiene puro (nunca lee el archivo por dentro).
 */
